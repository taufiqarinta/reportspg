<x-app-layout>
        ...
        <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form id="opnameForm" method="POST" action="{{ route('stockopname.update', $stockopname) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8 p-4 bg-gray-50 rounded-lg">
                                ...
                                <select id="toko_id" name="toko_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach($tokoList as $toko)
                                        <option value="{{ $toko->id }}" 
                                            {{ old('toko_id', $stockopname->toko_id) == $toko->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="tahun" name="tahun" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    @for($year = date('Y') - 1; $year <= date('Y') + 1; $year++)
                                        <option value="{{ $year }}" 
                                            {{ old('tahun', $stockopname->tahun) == $year ? 'selected' : '' }}>
                                            {{ $year }}
                                        </option>
                                    @endfor
                                </select>
                            ...
                            <div>
                                ...
                                <select id="bulan" name="bulan" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    @foreach([
                                        ...
                                        <option value="{{ $num }}" 
                                            {{ old('bulan', $stockopname->bulan) == $num ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="tanggal" 
                                            ...
                                            type="date" 
                                            name="tanggal" 
                                            :value="old('tanggal', $stockopname->tanggal->format('Y-m-d'))" 
                                            required />
                                    ...
                                    <table class="responsive-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="column-stock">Stock (Box)</th>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200" id="itemsBody">
                                            <!-- Items akan diisi dari data existing -->
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                            <div id="noItemsMessage" class="text-center py-8 text-gray-500" style="display: none;">
    ...
    $(document).ready(function() {
        ...
        function addNewRow(itemData = null) {
            ...
            // Jika ada data item, tambahkan ke selectedItems
            if (itemCode) {
                selectedItems.add(itemCode);
                
                // Pre-select item di Select2
                setTimeout(() => {
                    if (selectElement.val() !== itemCode) {
                        selectElement.val(itemCode).trigger('change');
                    }
                }, 100);
            }
        ...
        $('#submitBtn').on('click', function() {
            // Update semua index sebelum submit
            updateAllIndexes();
