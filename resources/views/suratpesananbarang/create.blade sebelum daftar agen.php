<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Surat Pesanan Barang') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('suratpesananbarang.store') }}" method="POST" id="spbForm">
                        @csrf
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">No. Surat</label>
                                <input type="text" value="{{ $nomorSurat }}" class="w-full border-gray-300 rounded-md shadow-sm bg-gray-100" readonly>
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Surat</label>
                                <input type="date" value="{{ $tanggalSurat }}" class="w-full border-gray-300 rounded-md shadow-sm bg-gray-100" readonly>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Pesanan Dari <span class="text-red-500">*</span></label>
                                <input type="text" name="pengirim" value="{{ old('pengirim') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Penerima Barang <span class="text-red-500">*</span></label>
                                <input type="text" name="penerima" value="{{ old('penerima') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Dikirim Ke <span class="text-red-500">*</span></label>
                                <input type="text" name="dikirim_ke" value="{{ old('dikirim_ke') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Kirim <span class="text-red-500">*</span></label>
                                <input type="date" name="tanggal_kirim" value="{{ old('tanggal_kirim', $tanggalKirim) }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Pemesan <span class="text-red-500">*</span></label>
                                <input type="text" name="pemesan" value="{{ old('pemesan') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="jenis_harga" id="jenisHarga" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    <option value="franco">Harga Franco</option>
                                    <option value="loco">Harga Loco</option>
                                </select>
                            </div>
                        </div>
                                ...
                                <div class="overflow-x-scroll">
                                    <table class="responsive-table">
                                        ...
                                        <tbody id="detailTableBody">
                                            <tr class="detail-row">
                                                <td class="column-no">1</td>
                                                <td class="column-ukuran">
                                                    <select name="details[0][ukuran]" class="w-full border-gray-300 rounded-md text-sm ukuran-select" onchange="loadProducts(this)" required>
                                                ...
                                                <td class="column-product">
                                                    <select name="details[0][nama_product]" class="w-full border-gray-300 rounded-md text-sm product-select" onchange="loadBrands(this)" required>
                                                        <option value="">Pilih Product</option>
                                                    </select>
                                                </td>
                                                <td class="column-brand">
                                                    <select name="details[0][brand]" class="w-full border-gray-300 rounded-md text-sm brand-select" onchange="loadKw(this)" required>
                                                        <option value="">Pilih Brand</option>
                                                    </select>
                                                </td>
                                                <td class="column-kw">
                                                    <select name="details[0][kw]" class="w-full border-gray-300 rounded-md text-sm kw-select" onchange="loadHarga(this)" required>
                                                        <option value="">Pilih KW</option>
                                                    </select>
                                                </td>
                                                <td class="column-jumlah">
                                                    <input type="number" step="1" name="details[0][jumlah_box]" class="w-full border-gray-300 rounded-md text-sm jumlah-box" oninput="calculateRowTotal(this)" required>
                                                </td>
                                                <td class="column-harga">
                                                    <input type="number" step="0.01" name="details[0][harga_satuan_box]" class="w-full border-gray-300 rounded-md text-sm harga-satuan" readonly>
                                                </td>
                                                <td class="column-disc">
                                                    <input type="number" step="1" name="details[0][disc]" class="w-full border-gray-300 rounded-md text-sm disc" oninput="calculateRowTotal(this)" value="0">
                                                </td>
                                                <td class="column-biaya">
                                                    <input type="number" step="1" name="details[0][biaya_tambahan_ekspedisi]" class="w-full border-gray-300 rounded-md text-sm biaya-ekspedisi" oninput="calculateRowTotal(this)" value="0">
                                                </td>
                                                ...
                                                <td class="column-keterangan">
                                                    <input type="text" name="details[0][keterangan]" class="w-full border-gray-300 rounded-md text-sm">
                                                </td>
                                        ...
                                        <tfoot>
                                            <tr class="bg-gray-50 font-bold">
                                                <td colspan="9" class="px-4 py-3 text-right">Total Keseluruhan:</td>
                                                <td class="px-4 py-3"><input type="text" id="totalKeseluruhan" class="w-full border-gray-300 rounded-md text-sm bg-gray-100" readonly value="0"></td>
                                                <td colspan="2"></td>
                                            </tr>
                                        </tfoot>
                                    </table>
