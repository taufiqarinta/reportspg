<x-app-layout>
        ...
        $(document).ready(function() {
            ...
            function addNewRow() {
                ...
                const newRow = `
                    <tr class="item-row" id="row-${rowIndex}">
                        ...
                        <td class="column-stock">
                            <input type="number" class="stock-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                   name="items[${rowIndex}][stock]" min="0" value="0" required
                                    onfocus="if(this.value=='0') this.value='';"
                                    onblur="if(this.value=='') this.value='0';">
                        </td>
                        <td class="column-masuk">
                            <input type="number" class="qty-masuk-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                   name="items[${rowIndex}][qty_masuk]" min="0" value="0" required
                                    onfocus="if(this.value=='0') this.value='';"
                                    onblur="if(this.value=='') this.value='0';">
                        </td>
                        <td class="column-catatan">
                            <input type="text" class="catatan-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                   name="items[${rowIndex}][catatan]" maxlength="500">
                        </td>
                        <td class="column-aksi">
            ...
            function updateRowIndexes() {
                $('#itemsBody tr').each(function(index) {
                    ...
                    row.find('input[name^="items["][name$="stock"]').attr('name', `items[${newIndex}][stock]`);
                    ...
                    success: function(response) {
                        if (response.success && response.data.length > 0) {
                            ...
                            response.data.forEach(function(item, index) {
                                        ...
                                        <td class="column-nama">
                                            <select class="item-select w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                                ...
                                                <option value="${item.item_code}" selected>
                                                    ${item.item_code} - ${item.nama_barang} - ${item.ukuran}
                                                </option>
                                            </select>
                                        </td>
                                        <td class="column-kode">
                                            <input type="text" class="itemcode-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-gray-100" 
                                                   name="items[${rowIndex}][item_code_display]" value="${item.item_code}" readonly>
                                        </td>
                                        <td class="column-ukuran">
                                            <input type="text" class="ukuran-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-gray-100" 
                                                   name="items[${rowIndex}][ukuran]" value="${item.ukuran}" readonly>
                                        </td>
                                        <td class="column-stock">
                                            <input type="number" class="stock-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                                   name="items[${rowIndex}][stock]" min="0" value="${item.stock}" required
                                                    onfocus="if(this.value=='0') this.value='';"
                                                    onblur="if(this.value=='') this.value='0';">
                                        </td>
                                        <td class="column-masuk">
                                            <input type="number" class="qty-masuk-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                                   name="items[${rowIndex}][qty_masuk]" min="0" value="${item.qty_masuk}" required
                                                    onfocus="if(this.value=='0') this.value='';"
                                                    onblur="if(this.value=='') this.value='0';">
                                        </td>
                                        <td class="column-catatan">
                                            <input type="text" class="catatan-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                                   name="items[${rowIndex}][catatan]" value="${item.catatan || ''}" maxlength="500">
                                        </td>
                                        <td class="column-aksi">
                                            <button type="button" class="remove-row-btn text-red-600 hover:text-red-900 text-sm" data-row="${rowIndex}">
                                                Hapus
                                ...
                                $('#itemsBody').append(newRow);
                                selectedItems.add(item.item_code);
                                
                                // Initialize Select2 for the row
                                const selectElement = $(`#row-${rowIndex} .item-select`);
                                selectElement.select2({
                                    placeholder: 'Cari barang...',
                                    allowClear: true,
                                    width: '100%',
                                    ...
                                    data: [{
                                        id: item.item_code,
                                        text: `${item.item_code} - ${item.nama_barang} - ${item.ukuran}`
                                    }],
                                    ajax: {
                                        url: '{{ route("api.stock-items") }}',
                                        dataType: 'json',
                                    ...
                                    templateResult: formatItem,
                                    templateSelection: formatItemSelection
                                }).val(item.item_code).trigger('change');
                            });
                            
                            $('#noItemsMessage').hide();
                            alert('Berhasil mengambil stock minggu sebelumnya. Total ' + response.data.length + ' item.');
                        } else {
                            alert('Tidak ada data stock sebelumnya untuk periode ini.');
                        }
                    },
                    error: function(xhr) {
                        alert('Terjadi kesalahan saat mengambil data stock.');
    ...
    $(document).ready(function() {
        ...
        function addNewRow(itemData = null) {
            ...
            let catatan = itemData ? itemData.catatan : '';
            ...
            const newRow = `
                <tr class="item-row" id="row-${rowIndex}">
                    ...
                    <td class="column-kode">
                        <input type="text" class="itemcode-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-gray-100" 
                               name="items[${rowIndex}][item_code_display]" value="${itemCode}" readonly>
                    </td>
                    <td class="column-ukuran">
                        <input type="text" class="ukuran-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-gray-100" 
                               name="items[${rowIndex}][ukuran]" value="${ukuran}" readonly>
                    </td>
                    <td class="column-stock">
                        <input type="number" class="stock-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                               name="items[${rowIndex}][stock]" min="0" value="${stock}" required>
                    </td>
                    <td class="column-masuk">
                        <input type="number" class="qty-masuk-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                               name="items[${rowIndex}][qty_masuk]" min="0" value="${qtyMasuk}" required>
                    </td>
                    <td class="column-catatan">
                        <input type="text" class="catatan-input w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                               name="items[${rowIndex}][catatan]" value="${catatan}" maxlength="500">
                    </td>
                    <td class="column-aksi">
                        <button type="button" class="remove-row-btn text-red-600 hover:text-red-900 text-sm" data-row="${rowIndex}">
                            Hapus
