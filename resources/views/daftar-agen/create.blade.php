        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('daftar-agen.store') }}">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="nama_agen" id="nama_agen" value="{{ old('nama_agen') }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 uppercase-input"
                                    oninput="this.value = this.value.toUpperCase()">
                                @error('nama_agen')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="alamat" id="alamat" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 uppercase-input"
                                    oninput="this.value = this.value.toUpperCase()">{{ old('alamat') }}</textarea>
                                @error('alamat')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
